# %%
# Parameter sets used in Wickramanayake et al. (2026)
import os
import pybamm
import numpy as np
...
D_c_nmc811_interpolated_data = pybamm.parameters.process_1D_data("stoichiometry_diffusion_interpolated_data_temp.csv", path=_DATA_DIR)
...
def nmc811_18650_diffusivity_Wickramanayake2026(c_s, T):
    """
    ...
    """
    c_max_pos = 49340.0
    sto = pybamm.surf(c_s) / c_max_pos
    name, (x, y) = D_c_nmc811_interpolated_data
    D_ref = pybamm.Interpolant(x, y, sto, name=name)
...
# %%
# Define parameter set for full cell model

param_dict_full_cell = {
    ...
    "Trapped lithium concentration in the shell [mol.m-3]": 20000,
    "Minimum concentration in positive electrode when fully discharged [mol.m-3]": 10953.48,
    "Minimum concentration in negative electrode when fully discharged [mol.m-3]": 68.514,
    "Threshold lithium concentration for phase transition [mol.m-3]": 15000,
    "Positive electrode shell resistivity [Ohm.m]": 1e6,
    "Constant oxygen concentration in the core [mol.m-3]": 152193.21,
    "Initial lithium concentration in positive core [mol.m-3]": 46478.28,
    "Initial oxygen concentration in positive shell [mol.m-3]": initial_oxygen_concentration,
    "Initial core-shell phase boundary location": 0.9868421,
    # cell
    ...
    "Positive electrode diffusivity [m2.s-1]": nmc811_18650_diffusivity_Wickramanayake2026,
    ...
    "Negative current collector surface heat transfer coefficient [W.m-2.K-1]": 10000,
    "Negative tab heat transfer coefficient [W.m-2.K-1]": 10000,
    "Positive current collector surface heat transfer coefficient [W.m-2.K-1]": 10000,
    "Positive tab heat transfer coefficient [W.m-2.K-1]": 10000,
    "Edge heat transfer coefficient [W.m-2.K-1]": 10000,
    "Negative Current collector surface heat transfer coefficient [W.m-2.K-1]": 0.5,
    "Positive Current collector surface heat transfer coefficient [W.m-2.K-1]": 0.5,
    "Total heat transfer coefficient [W.m-2.K-1]": 100,
    "Positive current collector thickness [m]": 1.6e-05,
    "Negative current collector thickness [m]": 1.2e-05,
    "Negative tab width [m]": 1.23,
    "Negative tab height [m]": 5.8e-2,
    "Positive tab width [m]": 1.23,
    "Positive tab height [m]": 5.8e-2,
    # citations
    "citations": ["Zhuo2023"],
    # s_dot + solvent oxidation
    "RS formation reaction coefficient [m.s-1]": 2.2e-12,
    "OCP for s_dot model [V]": 3.9,
    "Transfer coefficient for s_dot model": 0.05,
    "OCP for solvent oxidation [V]": 4.0,
    "Solvent oxidation reaction coefficient [m.s-1]": 3e-10,
    "Solvent oxidation transfer coefficient": 0.3,
    "Initial oxygen concentration in external tank [mol.m-3]": 1000,
}


param_full_cell = pybamm.ParameterValues(param_dict_full_cell)


# %%
# Define parameter set for half cell model
param_dict_half_cell = {
    "chemistry": "lithium_ion",
    # sei
    "Ratio of lithium moles to SEI moles": 2.0,
    "Inner SEI reaction proportion": 0.5,
    ...
    "Negative particle radius [m]": 3.8e-6,
    ...
    "Positive tab heat transfer coefficient [W.m-2.K-1]": 10000,
    "Edge heat transfer coefficient [W.m-2.K-1]": 10000,
    "Negative Current collector surface heat transfer coefficient [W.m-2.K-1]": 1,
    "Positive Current collector surface heat transfer coefficient [W.m-2.K-1]": 1,
    "Total heat transfer coefficient [W.m-2.K-1]": 100,
    "Positive current collector thickness [m]": 1.6e-05,
    "Negative current collector thickness [m]": 1.2e-05,
    "Negative tab width [m]": 1.23,
...
param_half_cell = pybamm.ParameterValues(param_dict_half_cell)